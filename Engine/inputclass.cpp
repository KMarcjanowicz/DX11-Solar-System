...
void InputClass::ProcessInput()
{
	...
	if (m_mouseX < 0) {
		mouseXDelta = m_mouseX - oldMouseX;
		oldMouseX = m_screenHeight + mouseXDelta;

		m_mouseX = m_screenHeight;
	}
	if (m_mouseY < 0) {
		mouseYDelta = m_mouseY - oldMouseY;
		oldMouseY = m_screenHeight + mouseYDelta;
	...
	if (m_mouseX > m_screenWidth) {
		mouseXDelta = m_mouseX - oldMouseX;
		m_mouseX = 0;
		oldMouseX = m_mouseX + mouseXDelta;

	}
	if (m_mouseY > m_screenHeight) {
		mouseYDelta = m_mouseX - oldMouseY;
		m_mouseY = 0;
		oldMouseY = m_mouseY + mouseYDelta;
	}
...
int InputClass::GetMouseXDelta()
{
	int MouseXDelta = m_mouseX - oldMouseX;
	oldMouseX = m_mouseX;

	return MouseXDelta;
}

int InputClass::GetMouseYDelta()
{
	int MouseYDelta = m_mouseY - oldMouseY;
	oldMouseY = m_mouseY;
